...
public class LiveService {
	...
	public void startLiveUpdatesForEvent(Event event) {
	...
	private Thread startNewThread(Event event){
		Thread thread = Thread.ofVirtual().start(() -> {
			while (!Thread.currentThread().isInterrupted()) {
				try {
					externalService.fetchEventInfo(event.getId()).ifPresent(info -> kafkaService.sendEventToKafka("live-events", info.toJson()));
	...
	public void stopLiveUpdatesForEvent(String eventId) {
